...
function ClaimRow({ description, link, claimedAt, claimFunction }) {
  return (
    <div className={styles.claimRow}>
      {claimedAt && <div><StatusSuccess className={styles.claimedIcon}/></div>}
      {!claimedAt && <Button className={styles.claimButton} onClick={() => claimFunction(link)}>Claim</Button>}
      <div>{description}</div>
    </div>
  );
}
...
export default function CapabilityClaims() {
  ...
  const { inProgress: claimInProgress, sendRequest: addClaim } = useSelfServiceRequest();
