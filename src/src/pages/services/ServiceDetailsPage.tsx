        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4 items-start">
          <div>
            ...
            {description && (
              <PageSection
                ...
                headlineTip={hintFor("Description")}
              >
                ...
                {app.deploymentSource && (
                  <div className="rounded-[6px] border border-divider bg-surface-muted grid grid-cols-[repeat(auto-fit,minmax(130px,1fr))] gap-x-5 gap-y-3 px-3.5 py-3">
                    ...
                    {app.deploymentSource.revision && (
                      <Def term="Revision">
                        <Code>
                          {app.deploymentSource.revision.slice(0, 12)}
                        </Code>
                      </Def>
                    )}
            ...
            {urls.length > 0 && (
              <PageSection
                ...
                headlineTip={hintFor("Exposed at")}
              >
                <div className="flex flex-col gap-2">
                  {urls.map((u, i) => {
            ...
            {(kafkaTopics.length > 0 || databases.length > 0) && (
              <PageSection
                ...
                headlineTip={hintFor("Dependencies")}
              >
                {kafkaTopics.length > 0 && (
                  <div className="mb-3.5">
                    <SectionLabel
                      className="mb-2 block"
                      tip={hintFor("Kafka topics")}
                    >
                      Kafka topics
                    </SectionLabel>
            ...
            <PageSection
              ...
              headlineTip={hintFor("Connections")}
            >
              <p className="text-[0.8125rem] text-secondary leading-[1.6] mb-3 mt-0">
                Runtime connections observed to and from this workload's
                services, overlaid on its structure. Dependency edges appear
          ...
          <div>
            ...
            <PageSection
              ...
              headlineTip={hintFor("At a glance")}
            >
              {app.deploymentSource?.tool && (
  ...
  return (
    <div className="border border-card rounded-[8px] px-4 py-2.5 min-w-[118px] bg-surface">
      ...
      <LabelText
        as="div"
        side="bottom"
        className="mt-1 font-mono text-[0.5625rem] tracking-[0.06em] uppercase text-muted"
        label={label}
      />
    </div>
  );
...
function Def({ term, children }: { term: string; children: React.ReactNode }) {
  return (
    <div>
      <LabelText
        as="dt"
        className="font-mono text-[10px] uppercase tracking-[0.06em] text-muted mb-1"
        label={term}
      />
...
function LabelText({
  ...
  /** Which side the hint opens on. Defaults to "top". */
  side?: React.ComponentProps<typeof HintTooltip>["side"];
}) {
  const tip = hintFor(label);
  const el = (
    <Comp
      className={cn(className, tip && "cursor-help w-fit")}
      {...(tip ? { tabIndex: 0 } : {})}
    >
      {label}
    </Comp>
