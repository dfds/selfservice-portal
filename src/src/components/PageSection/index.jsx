...
export function SectionContent({ title, children }) {
  return (
    <>
      {title && (
        <h3 className="text-base font-semibold text-[#002b45] dark:text-[#e2e8f0] mb-2">{title}</h3>
      )}
      <div>{children}</div>
    </>
  );
...
export default function PageSection({ id, headline, headlineChildren, children }) {
  return (
      ...
      <div className="bg-white dark:bg-[#1e293b] border border-[#d9dcde] dark:border-[#334155] rounded-[8px] overflow-hidden">
        ...
        <div className="p-4">
          ...
          {Object.keys(tabsContent).map(
            (index) =>
              selectedTab === index && <div key={index}>{tabsContent[index]}</div>,
          )}
          {footer}
        </div>
      </div>
