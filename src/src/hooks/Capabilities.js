...
export function useMessageContracts(topicDefinition) {
  const { inProgress, responseData, setErrorOptions, sendRequest } =
    useSelfServiceRequest();
  const [isLoadedContracts, setIsLoadedContracts] = useState(false);
  const [contractsList, setContractsList] = useState([]);

  const link = topicDefinition?._links?.messageContracts;
  ...
  useEffect(() => {
    if (responseData?.items.length >= 0) {
      setContractsList(responseData?.items || []);
    }
  }, [responseData]);

  useEffect(() => {
    if (setContractsList.length >= 0) {
      setIsLoadedContracts(true);
    }
  }, [contractsList]);
...
export function useConsumers(topicDefinition) {
  const { inProgress, responseData, setErrorOptions, sendRequest } =
    useSelfServiceRequest();
  const [isLoadedConsumers, setIsLoadedConsumers] = useState(false);
  ...
  useEffect(() => {
    if (responseData?.items.length >= 0) {
      setConsumersList(responseData?.items || []);
    }
  }, [responseData]);

  useEffect(() => {
    if (setConsumersList.length >= 0) {
      setIsLoadedConsumers(true);
    }
  }, [consumersList]);
