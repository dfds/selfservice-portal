  ...
  return {
    ...
    _links: {
      ...
      membershipApplications: {
        href: composeUrl(
          `/capabilities/${capability.id}/membershipapplications`,
        ),
        rel: "related",
    ...
    ...{
      _links: {
        self: {
          ...
          rel: "self",
          allow: ["GET", topic.__canDelete ? "DELETE" : ""].filter(
            (x) => x != "",
          ),
        },
        messageContracts: {
          ...
          rel: "related",
          allow: ["GET", topic.name.startsWith("pub.") ? "POST" : ""].filter(
            (x) => x != "",
          ),
        },
...
export function convertMembershipApplication(
  memebrshipApplication: MembershipApplication,
): any {
  const approvalAllow = [];
